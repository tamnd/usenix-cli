...
describes what users get. Pick a format with `-o`, or let usenix choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
usenix <command> -o table   # aligned columns for reading
usenix <command> -o jsonl   # one JSON object per line, for piping
usenix <command> -o json    # a single JSON array
usenix <command> -o csv     # spreadsheet friendly
usenix <command> -o tsv     # tab-separated
usenix <command> -o url     # just the URL column
usenix <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
usenix <command> --fields id,title,url
```
...
```bash
usenix <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
usenix <command>            # a table, because this is a terminal
usenix <command> | wc -l    # JSONL, because this is a pipe
```
